...
public:
	...
	// 2026-09-10: scope charge as 0..1, available to BOTH builds. The existing
	// GetHUDDamagePerc() computes the same thing but is inside #ifdef CLIENT_DLL,
	// and the RL sniper bot (game/server/tf/tf_sniper_bot.cpp) needs the charge
	// server-side to feed its policy observation. Note GetProgress() is NOT this
	// -- that returns the Hitman's Heatmaker rage meter and reads 0 on a stock
	// rifle, which silently fed the bot a constant 0 charge for its whole life.
	float GetScopeChargePerc( void ) const;
