...
//-----------------------------------------------------------------------------
// Purpose: Scope charge as 0..1. Same value as the client-only
//          GetHUDDamagePerc(), but built for both DLLs -- see the header.
//          Lives outside the CLIENT_DLL block so it can use the real
//          TF_WEAPON_SNIPERRIFLE_DAMAGE_MAX rather than duplicating it.
//-----------------------------------------------------------------------------
float CTFSniperRifle::GetScopeChargePerc( void ) const
{
	return m_flChargedDamage / TF_WEAPON_SNIPERRIFLE_DAMAGE_MAX;
}
